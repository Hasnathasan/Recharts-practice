...
const Navbar = () => {
  ...
  const routes = [
    ...
    return (
        <div>
        <button className="md:hidden h-full p-2 rounded bg-teal-600" onClick={() => setIsOpen(!isOpen)}>
          <span>
            {isOpen ? (
        ...
        <ul className={`md:flex transition-all text-white duration-500 h-full p-10 md:p-3 gap-14 absolute bg-teal-600 md:sticky ${ isOpen ? "left-0" : "-left-48"}`}>
